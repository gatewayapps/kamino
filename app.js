...
function initializeExtension() {
  ...
  const popup = $(
    Handlebars.templates.modal(context).replace(/(\r\n|\n|\r)/gm, "")
  );
  ...
  if (
    urlObj.url.indexOf(
      `${urlObj.organization}/${urlObj.currentRepo}/compare/`
    ) < 0 &&
    urlObj.url.indexOf(`${urlObj.organization}/${urlObj.currentRepo}/pull/`) <
      0 &&
    urlObj.url.indexOf(
      `${urlObj.organization}/${urlObj.currentRepo}/issues/new`
    ) < 0 &&
      ...
      item => {
        ...
        item.filters.forEach(f => {
          ...
          if (
            ...
            f.currentRepo === filter.currentRepo
          ) {
            exists = true;
...
function loadRepos() {
  ...
  if (token === "") {
    console.log(
      "disabling button because there is no Personal Access Token for authentication with Github"
    );
  ...
  $(".repoDropdown").append(
    '<li class="dropdown-header dropdown-header-used">Last Used</li>'
  );
  $(".repoDropdown").append(
    '<li class="dropdown-header dropdown-header-rest">The Rest</li>'
  );
  ...
  chrome.storage.sync.get(
    ...
    item => {
      ...
      if (item.mostUsed && item.mostUsed.length > 0) {
        ...
        $(".quickClone").text(
          `Clone to ${item.mostUsed[0].substring(
            item.mostUsed[0].indexOf("/") + 1
          )}`
        );
...
function getGithubIssue(repo, closeOriginal) {
  ...
  ajaxRequest(
    "GET",
    "",
    `https://api.github.com/repos/${urlObj.organization}/${
      urlObj.currentRepo
    }/issues/${urlObj.issueNumber}`
  ).then(issue => {
    // build new issue
    const newIssue = {
      title: issue.data.title,
      body: `From ${urlObj.currentRepo} created by [${issue.data.user.login}](${
        issue.data.user.html_url
      }) : ${urlObj.organization}/${urlObj.currentRepo}#${
        urlObj.issueNumber
      }\n\n${issue.data.body}`,
      body: `From ${urlObj.currentRepo} created by [${issue.data.user.login}](${
        issue.data.user.html_url
        ...
        urlObj.issueNumber
      }\n\n ${issue.data.body}, labels: ${issue.data.labels}`
    };
...
function commentOnIssue(repo, oldIssue, newIssue, closeOriginal) {
  ...
  ajaxRequest(
    ...
    `https://api.github.com/repos/${urlObj.organization}/${
      urlObj.currentRepo
    }/issues/${urlObj.issueNumber}/comments`
  ).then(response => {
    if (closeOriginal) {
