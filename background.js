...
chrome.webNavigation.onHistoryStateUpdated.addListener(details => {
  chrome.tabs.executeScript(
    null,
    { file: "jquery/jquery-3.2.0.min.js", runAt: "document_end" },
    j => {
      chrome.tabs.executeScript(
        null,
        ...
        h => {
          chrome.tabs.executeScript(
            null,
            { file: "template.js", runAt: "document_end" },
            h => {
              chrome.tabs.executeScript(
                null,
        ...
        if (item.goToList) {
          ...
          chrome.tabs.query({ currentWindow: true, active: true }, tabs => {
            ...
            setTimeout(() => {
              if (item.createTab) {
                chrome.tabs.create({
                  url: `https://github.com/${request.repo}/issues/${
                    request.issueNumber
                  }`,
                  selected: false
                });
              ...
              chrome.tabs.update(tabs[0].id, {
                url: `https://github.com/${request.organization}/${
                  request.oldRepo
                }${filter.filter}`,
                selected: true
              });
