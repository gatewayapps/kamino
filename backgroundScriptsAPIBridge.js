...
class EdgeBridgeDebugLog {
  ...
  DoActionAndLog(action, name, deprecatedTo, bridgedTo) {
    ...
    try {
      ...
      if (
        typeof deprecatedTo !== "undefined" &&
        typeof deprecatedTo !== "null"
      ) {
        this.warn(
          `API Call Deprecated - Name: ${name}, Please use ${deprecatedTo} instead!`
        );
        this.AddToCalledDictionary(this.DeprecatedCalls, name);
      }
      if (typeof bridgedTo !== "undefined" && typeof bridgedTo !== "null") {
        this.info(
          `API Call '${name}' has been bridged to another Edge API: ${bridgedTo}`
        );
          ...
          if (typeof details.path === "object") {
            for (var key in details.path) {
              if (details.path.hasOwnProperty(key)) {
                details.path[key] = bridgeHelper.toAbsolutePath(
                  details.path[key]
                );
              }
            }
...
class EdgeChromeExtensionBridge extends EdgeExtensionBridge {
  ...
  sendMessage(message, responseCallback) {
    return bridgeLog.DoActionAndLog(
      () => {
        return EdgeRuntimeBridge.prototype.sendMessage(
          message,
          responseCallback,
          undefined,
          undefined
  ...
  constructor() {
    ...
    this.history =
      typeof browser.history !== "undefined"
        ...
        : undefined;
    this.i18n =
      typeof browser.i18n !== "undefined" ? new EdgeI18nBridge() : undefined;
    this.notifications =
      typeof browser.notifications !== "undefined"
        ? new EdgeNotificationBridge()
        : undefined;
    this.pageAction =
